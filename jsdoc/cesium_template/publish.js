  ...
  if (items && items.length) {
    ...
    items.forEach(item => {
      ...
      if ( !hasOwnProp.call(item, 'longname') ) {
        itemsNav += `<li data-name="${item.name}">${linktoFn('', item.name)}</li>`;
        ...
        itemsNav += `<li data-name="${item.name}">${linktoFn(item.longname, displayName.replace(/\b(module|event):/g, ''))}</li>`;
  ...
  if (members.globals.length) {
    ...
    members.globals.forEach(({kind, longname, name}) => {
      if ( kind !== 'typedef' && !hasOwnProp.call(seen, longname) ) {
        globalNav += `<li data-name="${name}">${linkto(longname, name)}</li>`;
      }
      seen[longname] = true;
    });
    ...
    if (!globalNav) {
      ...
      nav += `<h2>${linkto('global', 'Global')}</h2>`;
    } else {
...
exports.publish = function (taffyData, opts, tutorials) {
  ...
  data().each(function (doclet) {
    ...
    if (doclet.meta) {
      ...
      if (docletPath && conf["sourceUrl"]) {
        doclet.meta.filename = docletPath;
        doclet.meta.sourceUrl = conf["sourceUrl"]
          .replace("{version}", require('../../package.json').version) // Read version from main package json version
